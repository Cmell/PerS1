...
jsPsych.plugins['vsl-grid-scene'] = (function() {
  ...
  plugin.generate_stimulus = function(pattern, image_size) {
    // Image size can now be a grid, with each element corresponding to the
    // image in the grid.

    var nrows = pattern.length;
    var ncols = pattern[0].length;

    if (!Array.isArray(image_size[0])) {
      // it is one dimensional, so construct the appropriate array.
      var newSize = [];
      for (var row = 0; row < nrows; row++) {
        var tempCol = [];
        for (var col = 0; col < ncols; col++) {
          tempCol.push(image_size);
        }
        newSize.push(tempCol);
      }
      image_size = newSize;
    }
